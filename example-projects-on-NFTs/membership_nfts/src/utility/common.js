...
import { confirmTransactionFromFrontend,confirmTransactionFromBackend } from './shyft';
...
export async function signAndConfirm(network,encodedTransaction,privateKey,callback)
{
    const phantom = new PhantomWalletAdapter();
    await phantom.connect();
    const rpcUrl = clusterApiUrl(network);
    ...
    const ret = await confirmTransactionFromBackend(network,encodedTransaction,privateKey);
    console.log(ret);
